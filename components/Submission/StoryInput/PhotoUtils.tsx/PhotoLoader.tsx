...
const ACCEPTED_FILE_TYPES = ['image/jpeg', 'image/png', 'image/tiff', 'image/gif', 'image/bmp']
...
export const PhotoLoader: React.FC<PhotoLoaderProps> = ({
  ...
  const onDrop = useCallback((acceptedFiles: any[]) => {
    if (!acceptedFiles || !acceptedFiles.length) {
      ...
      setError(`That file type isn't allowed. Please select a jpg, png, tiff, gif, or bmp file. (You selected ${acceptedFiles[0].name}, ${acceptedFiles[0].type} file)`)
    } else {
  ...
  useEffect(() => {
    if (db && storyId?.length) {
      db.submissions.get(0).then((entry) => {
        ...
        if (cachedContent) {
          setPhotoSource({ //@ts-ignore
            url: URL.createObjectURL(cachedContent), type: cachedContent.type
          })
        }
      })
    }
  ...
  return (
    <Box sx={{ background: colors.darkgray, padding: '1em', border: `1px solid ${colors.gray}` }}>
      ...
      {error && <Box sx={{ borderLeft: `2px solid ${colors.red}`, padding: '0 0 0 1em', marginBottom: '2em' }}>
        <p>
          <span className="material-icons" style={{ color: colors.red }}>error</span>
          <br />
          {error}
        </p>
      </Box>}
      <Button {...getRootProps()} sx={{ border: `1px solid ${colors.yellow}`, textTransform: 'none', width: '100%', minHeight: '100px' }}>
        <input {...getInputProps()} />
        {
          isDragActive
            ? <p>
            ...
            : <p>
              ...
              <br />
              <i>Supported file types: jpg/jpeg, png, gif, bmp, tif/tiff</i>
            </p>
        }
      </Button>
    </Box>
