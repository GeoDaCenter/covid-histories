...
const surveyFetcher = (url: string) =>
	fetch(url)
		.then((res) => res.json())
...
const SurveyForm: React.FC<{
	...
	const { user } = useUser()
	const { data: hasCompletedSurvey } = useSWR(
		'/api/files/completed_survey',
		surveyFetcher
	)
		...
		<Box component="form" autoComplete="off" noValidate>
			...
			<PaddedFormGroup>
				<Typography variant="h6">
					What is your <u>self-identified</u> race and/or ethnicity?{' '}
					<i>Select all that apply:</i>
				</Typography>
				{raceOptions.map((name, i) => (
					<Grid
						key={`${name}-checkbox-self-${i}`}
						container
						spacing={1}
					...
					>
						<Grid item xs={12} md={4}>
							<FormControlLabel
								control={
									<Checkbox checked={selfIdentifiedRaces.includes(name)} />
								}
								label={name}
								onChange={() =>
									dispatch(toggleRace({ name, type: 'selfIdentifiedRace' }))
								}
							/>
						</Grid>
						<Grid item xs={12} md={8}>
							{selfIdentifiedRaces.includes(name) && (
								<TextField
									...
									maxRows={4}
									value={
										selfIdentifiedRace.find((f) => f.name === name)
											?.description || ''
									}
			...
			<PaddedFormGroup>
				<Typography variant="h6">
					How do you think <u>others perceive</u> your race and/or ethnicity?{' '}
					<i>Select all that apply:</i>
				</Typography>
				{raceOptions.map((name, i) => (
					<Grid
						key={`${name}-checkbox-perc-${i}`}
						container
						spacing={1}
						sx={{ marginBottom: '.25em' }}
					>
						<Grid item xs={12} md={4}>
							<FormControlLabel
								...
								onChange={() =>
									dispatch(
										toggleRace({ name, type: 'perceivedIdentifiedRace' })
									)
								}
							/>
						</Grid>
						<Grid item xs={12} md={8}>
							{perceivedRaces.includes(name) && (
								<TextField
									id="outlined-multiline-flexible"
									label={`Please describe (${name})`}
									multiline
            ...
            How old are you?
				...
				<TextField
					...
					value={age}
					onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
						!isNaN(+e.target.value) && handleAge(+e.target.value)
					}
				/>
			</PaddedFormGroup>
			<PaddedFormGroup>
				...
				<TextField
					...
					onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
						handleTextChange('genderIdentity', e.target.value)
					}
				/>
			</PaddedFormGroup>
			<PaddedFormGroup>
            ...
            How would you describe the place where you live? 
				...
				<FormControl>
					<InputLabel id="urbanicity-label">
						How would you describe the place where you live?
					</InputLabel>
					<Select
						...
						onChange={(e: SelectChangeEvent) =>
							handleTextChange('placeUrbanicity', e.target.value)
						}
					>
            ...
            Is there anything more you’d like to share about yourself?
				...
				<TextField
					...
					onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
						handleTextChange('additionalDescription', e.target.value)
					}
				/>
			</PaddedFormGroup>
			{allowSubmit && (
