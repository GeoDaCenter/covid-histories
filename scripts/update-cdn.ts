...
import { CloudFrontClient, CreateInvalidationCommand, CreateInvalidationCommandInput } from "@aws-sdk/client-cloudfront"; // ES Modules import
import { getFileList, getObjectTags, deleteObject, copyObject, getPresignedUrl, putObject } from '../pages/api/files/utils'
...
async function main() {
	...
	for (let i=0; i<uploadContents.length; i++) {
        ...
        const publicFile = publicContents.find(({ id: publicId }) => publicId === id)
        const approvalStatus = TagSet?.find(f => f.Key === 'approved')?.Value
		
		if (approvalStatus === 'true') publicFiles.push({id: id!, Key: Key!, fileType:fileType!})
	...
	const fileIndexPromises: Promise<PublicSubmission>[] = publicFiles.map(async ({ id, Key, fileType }) => {
		const existingEntry: PublicSubmission | undefined = existingMeta?.find(({ id: existingId }) => existingId === id)
